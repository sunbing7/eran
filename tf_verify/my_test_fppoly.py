...
lpp = (lexpr_weights.__array_interface__['data'][0]+ np.arange(lexpr_weights.shape[0])*lexpr_weights.strides[0]).astype(np.uintp) #pointer points to lexpr_weights
print(lpp)
...
np.ascontiguousarray(lexpr_cst, dtype=np.double) #return as a continuous array in memory
